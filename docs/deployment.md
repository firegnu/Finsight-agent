...
# 4. 配置 .env（首次；至少设 DEEPSEEK_API_KEY + LLM_EMBEDDING_API_KEY）
cp .env.example .env
$EDITOR .env
...
| 变量 | 开发模式 | 生产模式 |
|---|---|---|
| `DEFAULT_PROVIDER_ID` | `deepseek` 或 `lmstudio` | 建议 `deepseek`（快、便宜，VPS 无本地模型）|
| `CORS_ORIGINS` | 留默认 | 单进程部署**无关紧要**（同源），但 Docker/多域名需要改 |
| `DEEPSEEK_API_KEY` | 可选 | **必填**（或删掉这个 provider） |
| `ZHIPU_API_KEY` | 可选 | 可选（作为慢速免费 fallback）|
...
2. **VPS 上手写**：`cp .env.example .env && vi .env`，填 `DEEPSEEK_API_KEY` + `LLM_EMBEDDING_API_KEY`
...
# {"status":"ok","model":"deepseek-chat","provider":"deepseek",...}

docker compose ps
# STATUS 应该是 "Up X seconds (healthy)"
```

浏览器打开 `http://<vps-public-ip>:8000`，发一个查询，一般 30-60 秒出报告（DeepSeek V3 默认档速度；若切到 Zhipu 免费档会慢到 2-3 分钟，WaitingIndicator 会实时告诉你在哪一步）。
...
A: 是的，这是免费档固有延迟（10-45s / 轮，多轮累加 2-3 分钟）。非 bug。**默认 provider 已切到 DeepSeek V3**（每轮 2-5s，完整分析 30-60s，成本 ~¥0.03/次）；Zhipu 免费档保留作 fallback。前端 `WaitingIndicator` 会实时显示等待秒数。
